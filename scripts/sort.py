...
def override_header(path: Path, recipe: CommentedMap) -> list[str] | None:
    if path.resolve().parent != OVERRIDE_DIR:
        return None
...
def process_recipe(path: Path, check: bool) -> bool:
    ...
    header = override_header(path, normalized)
    if header:
        normalized.yaml_set_start_comment("\n".join(header))
    expected_header = ["---", *(f"# {line}" for line in header or [])]
    header_matches = header is None or original.splitlines()[:3] == expected_header
    if mapping_order(recipe) == mapping_order(normalized) and header_matches:
        return True
    if not check:
        output = StringIO()
